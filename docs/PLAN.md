...
> ⚠️ На AVD перед проверкой FAB «моё местоположение» открыть Extended Controls → Location и выставить точку в Сочи (например, 43.585, 39.723). Дефолтная локация эмулятора — Mountain View, и FusedLocationProvider честно отдаст её → камера улетит в Калифорнию.
...
- [x] `ComplaintsPage` (главный экран — таблица + детали справа)
- [x] Фильтры: статус (включая REJECTED/DUPLICATE для админа), 18 категорий, район, SLA
- [x] Сортировка по приоритету / дате / голосам
- [x] Detail-panel справа (как в мокапе): фото, мета, голоса, карта, описание, история статусов
- [x] Действия: «Принять в работу» / «Решить» / «Отклонить» / «Дубликат»
  - [x] Обязательное поле комментария
  - [x] Для DUPLICATE — выбор оригинала из списка близких
- [x] Счётчики в фильтрах обновляются после действий
- [ ] Yandex Maps JS API на детальной карте — отложено (нужен JS API ключ)
...
- [x] **Управление командой в Settings:**
  ...
  - [x] Кнопки **«Заморозить»** / **«Разморозить»** в табах Активные/Замороженные. Защита: I1 (не сам себя), I2 (≥1 активный ADMIN остаётся), I5 (отзыв refresh-токенов).
  - [x] Pending invitations в отдельном табе + кнопка отзыва.
