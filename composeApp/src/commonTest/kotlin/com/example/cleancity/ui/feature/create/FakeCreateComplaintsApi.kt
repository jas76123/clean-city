package com.example.cleancity.ui.feature.create

import com.example.cleancity.data.network.ComplaintsApiContract
import com.example.cleancity.domain.photo.PhotoBytes
import com.example.cleancity.shared.models.ComplaintListResponse
import com.example.cleancity.shared.models.ComplaintResponse
import com.example.cleancity.shared.models.DuplicateCandidatesResponse
import com.example.cleancity.shared.models.MapMarkersResponse
import com.example.cleancity.shared.models.ProblemCategory
import com.example.cleancity.shared.models.VoteResponse
import com.example.cleancity.shared.requests.CreateComplaintRequest

class FakeCreateComplaintsApi : ComplaintsApiContract {

    data class CreateCall(val request: CreateComplaintRequest, val photos: List<PhotoBytes>)
    data class DuplicateCall(val lat: Double, val lon: Double, val cat: ProblemCategory, val r: Int?)

    val createCalls = mutableListOf<CreateCall>()
    val duplicateCalls = mutableListOf<DuplicateCall>()
    val voteCalls = mutableListOf<Long>()

    var nextCreateResult: Result<ComplaintResponse>? = null
    var nextDuplicatesResult: Result<DuplicateCandidatesResponse> =
        Result.success(DuplicateCandidatesResponse(emptyList()))
    var nextVoteResult: Result<VoteResponse>? = null

    override suspend fun create(request: CreateComplaintRequest, photos: List<PhotoBytes>): ComplaintResponse {
        createCalls += CreateCall(request, photos)
        return requireNotNull(nextCreateResult) { "nextCreateResult not set" }.getOrThrow()
    }

    override suspend fun findDuplicates(
        latitude: Double, longitude: Double,
        category: ProblemCategory, radiusMeters: Int?,
    ): DuplicateCandidatesResponse {
        duplicateCalls += DuplicateCall(latitude, longitude, category, radiusMeters)
        return nextDuplicatesResult.getOrThrow()
    }

    override suspend fun vote(id: Long): VoteResponse {
        voteCalls += id
        return requireNotNull(nextVoteResult) { "nextVoteResult not set" }.getOrThrow()
    }

    override suspend fun unvote(id: Long): VoteResponse = error("not used")

    override suspend fun getMapMarkers(
        swLat: Double, swLon: Double, neLat: Double, neLon: Double,
        category: ProblemCategory?,
    ): MapMarkersResponse = error("not used")

    override suspend fun list(
        page: Int, size: Int, sort: String,
        category: ProblemCategory?, district: String?,
    ): ComplaintListResponse = error("not used")

    override suspend fun mine(page: Int, size: Int): ComplaintListResponse = error("not used")
    override suspend fun voted(page: Int, size: Int): ComplaintListResponse = error("not used")
    override suspend fun getById(id: Long): ComplaintResponse = error("not used")
}
